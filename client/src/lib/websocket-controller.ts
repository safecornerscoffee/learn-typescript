...
export class WebsocketController {
  ...
  connect(messagesCallback: (message:Message) => void): Promise<WebSocket> {
  ...
  async send(message: Partial<Message>, awaitForReply: boolean = false): Promise<Message> {
    return new Promise<Message>(async (resolve, reject) => {
      if (awaitForReply) {
        this.messagesAwaitingReply.set(message.correlationId!, { resolve , reject } as PromiseExecutor<Message>);
      }
      this.websocket.then(
